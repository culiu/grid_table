...
$SQL = "SELECT invid, invdate, amount, tax, email, note FROM invheader ORDER BY $sidx $sord LIMIT $start , $limit"; 
...
while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
  ...
  $responce->rows[$i]['cell']=array('',$row[invid],$row[invdate],$row[amount],$row[tax],$row[email], $row[note],'');
  $i++;
}
